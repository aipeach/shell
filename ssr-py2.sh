...
if [[ ${release} == "centos" ]]; then
    yum install epel-release yum update yum install libffi libffi-devel openssl-devel python2-pip git -y
    yum install net-tools libsodium -y
    systemctl stop firewalld
    systemctl disable firewalld
else
    apt install python-pip libffi-dev libssl-dev git libsodium-dev -y
fi
...
git clone -b py2 https://github.com/Anankke/shadowsocks /opt/shadowsocks/default
...
pip install -r /opt/shadowsocks/default/requirements.txt
...
echo "ExecStart=/usr/bin/python /opt/shadowsocks/%i/server.py" >> /etc/systemd/system/shadowsocks@.service
